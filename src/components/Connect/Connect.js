...
const Connect = () => {
    ...
    const details =services.filter((s)=>s.name==serviceId)
    console.log(details)
    ...
    <div className="card shadow-lg w-100 h-100 text-center rounded vehicleCard ">
        ...
        <h3 className="text-warning fw-bold">Services name: {details[0]?.name}</h3>
            <p className="text-primary">{details[0]?.name}</p>
            <p className="text-primary">{details[0]?.email}</p>
        </div>
        <div className="card-footer">
            <div className="d-flex align-items-center justify-content-between ">
                <h3 className="text-warning fw-bold"> Degree $ {details[0]?.degree}</h3>
